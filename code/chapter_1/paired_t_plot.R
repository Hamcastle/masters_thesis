...
#Load the data
error_t_test_data<-data.frame(read.csv("~/Dropbox/neu_experiments/oculotrain_experiment_phase1_orientation/out/by_subject_median_error_data.csv"))
stability_t_test_data<-data.frame(read.csv("~/Dropbox/neu_experiments/oculotrain_experiment_phase1_orientation/out/gaze_isocontour_data.csv"))

#Filter it
abs_block_num_filter<-c(1,5)
...
summary_error_data<-summarySEwithin(filtered_error_data,measurevar="error_z_scores",withinvars="abs_block_num",idvar="subject_id",conf.interval=.95)
...
#Save the figure
ggsave(t_test_plot,file='~/Dropbox/masters_document/figures/chapter_1/paired_t_results.pdf',units=c("mm"),width=120.65,height=100,dpi=1000)